...
Створити об'єкт МП
    ...
    Run keyword    sets.Змінити дані профілю органу приватизації    ${tender_data.data.assetCustodian}
    ...
    Run keyword    sets.Оновити сторінку з об'єктом МП    ${username}    ${assetID}
    [Return]    ${assetID}
...
Пошук лоту по ідентифікатору
    [Arguments]    ${username}    ${tender_uaid}
    Run keyword    sets.Оновити сторінку з лотом    ${username}    ${tender_uaid}
...
Завантажити ілюстрацію в об'єкт МП
    ...
    Run keyword    sets.Пошук об’єкта МП по ідентифікатору    ${username}    ${tender_uaid}
...
Завантажити ілюстрацію в лот
    [Arguments]    ${username}    ${tender_uaid}    ${filepath}
    Run keyword    sets.Пошук лоту по ідентифікатору    ${username}    ${tender_uaid}
...
Завантажити документ в умови проведення аукціону
    ...
    Run keyword    sets.Завантажити документ в лот з типом    ${username}    ${tender_uaid}    ${filepath}    ${documentType}

Внести зміни в об'єкт МП
    [Arguments]    ${username}    ${tender_uaid}    ${field_name}    ${field_value}
    Run keyword    sets.Змінити ${field_name} об'єкта МП    ${username}    ${tender_uaid}    ${field_value}

Внести зміни в лот
    [Arguments]    ${username}    ${tender_uaid}    ${fieldname}    ${fieldvalue}
    Run keyword    sets.Змінити ${fieldname} лоту    ${username}    ${tender_uaid}    ${field_value}

Внести зміни в актив лоту
    [Arguments]    ${username}    ${item_id}    ${tender_uaid}    ${fieldname}    ${fieldvalue}
    Run keyword    sets.Змінити поле ${fieldname} активу лоту    ${field_value}

Внести зміни в умови проведення аукціону
    [Arguments]    ${username}    ${tender_uaid}    ${fieldname}    ${fieldvalue}    ${auction_index}
    Run keyword    sets.Змінити поле ${fieldname} аукціону ${auction_index}    ${field_value}
...
Внести зміни в актив об'єкта МП
    ...
    Run keyword    sets.Змінити ${field_name} об'єкта МП    ${username}    ${tender_uaid}    ${item_id}    ${field_value}
...
Додати актив до об'єкта МП
    ...
    Run keyword    sets.Додати предмет    ${item}
...
Додати умови проведення аукціону
  ...
  Run KeyWord  sets.Додати умови проведення аукціону номер ${index}  ${username}  ${tender_uaid}  ${auction}

Додати умови проведення аукціону номер 0
  [Arguments]  ${username}  ${tender_uaid}  ${auction}
  Log    ${auction}
  sets.Оновити сторінку з лотом    ${username}  ${tender_uaid}
  Натиснути    id=auction-0-update-btn
...
Отримати інформацію з активу лоту
    [Arguments]    ${username}    ${tender_uaid}    ${item_id}    ${field_name}
    ${return_value}=    Run keyword     sets.Отримати значення поля ${field_name} щодо активу лоту    ${username}    ${tender_uaid}    ${item_id}
    [Return]    ${return_value}

Отримати інформацію із лоту
    [Arguments]    ${username}    ${tender_uaid}    ${field_name}
    ${return_value}=    Run keyword    sets.Отримати інформацію щодо лоту про ${field_name}
    [Return]    ${return_value}
...
Пошук тендера по ідентифікатору
    [Arguments]    ${username}    ${tender_uaid}
    Run keyword    sets.Оновити сторінку з тендером    ${username}    ${tender_uaid}

Отримати інформацію із тендера
    [Arguments]    ${username}    ${tender_uaid}    ${field_name}
    Run keyword    sets.Оновити сторінку з тендером    ${username}    ${tender_uaid}
    ${return_value}=    Run keyword    sets.Отримати значення поля ${field_name} тендеру
    [Return]    ${return_value}
...
Подати цінову пропозицію
    ...
    Run keyword    sets.Змінити дані профілю учасника    ${bid.data.tenderers[0]}
    Run keyword    sets.Пошук тендера по ідентифікатору    ${username}    ${tender_uaid}
...
Отримати інформацію із документа
    ...
    ${return_value}=    Run keyword    sets.Отримати значення поля ${field} документу    ${doc_id}
    [Return]    ${return_value}
...
Отримати інформацію із пропозиції
    ...
    Run keyword    sets.Пошук тендера по ідентифікатору    ${username}    ${tender_uaid}
    Натиснути    id=bid-create-btn
    ${return_value}=    Run keyword    sets.Отримати інформацію з пропозиції шодо ${field}
    [Return]    ${return_value}

sets.Отримати інформацію з пропозиції шодо value.amount
